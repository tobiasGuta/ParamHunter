...
public class FuzzingEngine {
    ...
    /**
     * Send a request with exponential backoff on 429 responses.
     */
    private HttpRequestResponse sendWithRetry(HttpRequest request) {
        ...
        for (int attempt = 0; attempt <= maxRetries; attempt++) {
            ...
            try {
                api.logging().logToOutput("[ParamHunter]   sendRequest attempt " + (attempt + 1)
                ...
                api.logging().logToOutput("[ParamHunter]   got " + rr.response().statusCode()
                        + " (" + (rr.response().body() != null ? rr.response().body().length() : 0) + " bytes)");
                if (rr.response().statusCode() == 429) {
                    if (attempt < maxRetries) {
                        String msg = "Rate limited (429) on " + target
                                + ", backing off " + backoff + "ms (retry "
